...
def train_audio_video(epoch, train_loader, model, optimizer, merge_alpha=0.5):
    ...
    for step, (spectrogram, image, y) in enumerate(train_loader):
        ...
        # audio
        out_a, o_fea, add_fea = model.classfier(o_a, w[:, 0][0], is_a=True)
        if add_fea is None:
            loss_a = criterion(out_a, y).mean()
        else:
            kl = y * o_fea.detach().softmax(1)
            loss_a = criterion(out_a, y).mean() + criterion(o_fea, y).mean() + criterion(add_fea, y).mean() - 0.2 * criterion(add_fea, kl).mean() # criterion(add_fea, y-0.2*kl).mean() = criterion(add_fea, y).mean() - 0.2 * criterion(add_fea, kl).mean()
        loss_a.backward()
        optimizer.step()
        optimizer.zero_grad()
        # video
        out_v, o_fea, add_fea = model.classfier(o_v, w[:, 1][0], is_a=False)
        if add_fea is None:
            loss_v = criterion(out_v, y).mean()
        else:
            kl = y * o_fea.detach().softmax(1)
            loss_v = criterion(out_v, y).mean() + criterion(o_fea, y).mean() + criterion(add_fea, y).mean() - 0.2 * criterion(add_fea, kl).mean() # criterion(add_fea, y-0.2*kl).mean() = criterion(add_fea, y).mean() - 0.2 * criterion(add_fea, kl).mean()
        loss_v.backward()
        optimizer.step()
        optimizer.zero_grad()
...
def val(epoch, val_loader, model, merge_alpha=0.5):
    ...
    with torch.no_grad():
        for step, (spectrogram, image, y) in enumerate(val_loader):
            ...
            out_a, _, _ = model.classfier(o_a, w[:, 0][0], is_a=True)
            out_v, _, _ = model.classfier(o_v, w[:, 1][0], is_a=False)
    ...
    print(('s_a:{s_a:.3f}, s_v:{s_v:.3f}, w:{w}').format(s_a=s_a, s_v=s_v, w=w))
    return acc

if __name__ == '__main__':
    # ----- LOAD PARAM -----
    parser = argparse.ArgumentParser()
    parser.add_argument('--config',type=str, default='/data/kinetics_sound.json')
    ...
    for epoch in range(cfg['train']['epoch_dict']):
        ...
        if acc > best_acc:
            ...
            torch.save(model.state_dict(), ('KS_GB_best_model_{acc:.4f}.pth').format(acc=best_acc))
